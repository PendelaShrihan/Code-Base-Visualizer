...
from parser.repo_walker import graph_to_json, scan_repository
...
def process_repository_task(repo_url: str) -> dict[str, Any]:
    ...
    Celery task that orchestrates the end-to-end repository parsing pipeline:
      1. Clones the remote git repository to a temporary directory.
      2. Recursively scans Python files and extracts the AST / dependency graph.
      3. Serializes the NetworkX graph into a JSON-serializable dict.
      4. Cleans up the temporary clone directory.
      5. Returns the graph dictionary.
    ...
    try:
        ...
        # Step 2: Scan repository and build graph
        ...
        # Step 3: Serialize graph to JSON-friendly dictionary
